    ...
    */
    ...
    $('.f1 .btn-next').on('click', function() {
        ...
        if(($("input[name='usersex']:checked").length === 0)  ){
         ...
         var texterrtime = document.getElementById("texterrtime");
        var substrtime = ['rankradio1','rankradio2', 'rankradio3', 'rankradio4','rankradio5','rankradio6'];
        $.each(substrtime , function(index, timea) { 
                        if($("input[name="+timea+"]:checked").length === 0){
                        next_step = false;
						texterrtime.style.display = "block";
                        }
                else {
            
                texterrtime.style.display = "none";
            }
        });
